...
public class Service extends Thread {
	...
	// 서비스 전에 실행되는게 서버클래스 그때 이미 vector를 만들어 놨었다.
	// 서버가 가지고 있는 정보를 가져오는게 바로 public Service(Socket s, Server server) 이다.
		...
		try {
			...
			while (true) {
				...
				if (msg.trim().length() > 0) {
					...
					switch (protocol) {
					// 케이스 수가 많다는 것은 클라이언트가 보내는 메세지의 종류가 많아졌다는 것이다.
					case "100": // 대기실 접속
						allV.add(this);// 전체사용자에 등록 , this<-- Service.class
					...
					case "150": // 대화명 입력
						...
						messageWait("160|" + getRoomInfo()); // 개설된 방의 정보를 방정보에 넘긴다. 160== view
						// out.writr( 방정보 )
						messageWait("180|" + getWaitInwon()); // 대기인원을 대기실 정보에 준다. 180==대기실
						// z클라이언트에게 전달된 메세지를 JList 방정보에 뿌린다.
						// out.write( 180| 사용자들 )
						break;
